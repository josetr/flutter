...
// FlVulkanManager manages Vulkan resources for the Linux embedder's
// Skia+Vulkan path.
...
static VkPresentModeKHR ChoosePresentMode(
  ...
  const gchar* requested = g_getenv("FLUTTER_VULKAN_PRESENT_MODE");
  if (requested != nullptr) {
    VkPresentModeKHR requested_mode = VK_PRESENT_MODE_FIFO_KHR;
    bool has_requested_mode = true;
    if (g_ascii_strcasecmp(requested, "immediate") == 0) {
      requested_mode = VK_PRESENT_MODE_IMMEDIATE_KHR;
    } else if (g_ascii_strcasecmp(requested, "mailbox") == 0) {
      requested_mode = VK_PRESENT_MODE_MAILBOX_KHR;
    } else if (g_ascii_strcasecmp(requested, "fifo") == 0) {
      requested_mode = VK_PRESENT_MODE_FIFO_KHR;
    } else {
      has_requested_mode = false;
      FML_LOG(WARNING) << "Ignoring invalid FLUTTER_VULKAN_PRESENT_MODE="
                       << requested;
    }

    if (has_requested_mode &&
        std::find(modes.begin(), modes.end(), requested_mode) != modes.end()) {
...
struct _FlVulkanManager {
  ...
  // Swapchain management for the embedder callbacks.
...
static bool check_device_extension(vulkan::VulkanProcTable& vk,
  ...
  uint32_t count = 0;
  VkResult result =
      enumerate_device_extension_properties(device, nullptr, &count, nullptr);
  if (result != VK_SUCCESS || count == 0) {
    return false;
  }

  std::vector<VkExtensionProperties> extensions(count);
  result = enumerate_device_extension_properties(device, nullptr, &count,
                                                 extensions.data());
  if (result != VK_SUCCESS) {
    return false;
  }
...
static bool load_instance_wsi_procs(FlVulkanManager* self) {
  ...
  self->destroy_surface_khr = reinterpret_cast<PFN_vkDestroySurfaceKHR>(
  ...
  return self->get_physical_device_surface_support_khr != nullptr &&
         self->get_physical_device_surface_capabilities_khr != nullptr &&
         self->get_physical_device_surface_formats_khr != nullptr &&
         self->get_physical_device_surface_present_modes_khr != nullptr &&
         self->destroy_surface_khr != nullptr;
}
...
static bool load_device_wsi_procs(FlVulkanManager* self) {
  ...
  self->destroy_swapchain_khr = reinterpret_cast<PFN_vkDestroySwapchainKHR>(
      self->vk->AcquireProc("vkDestroySwapchainKHR", device_handle));
  self->get_swapchain_images_khr =
      reinterpret_cast<PFN_vkGetSwapchainImagesKHR>(
          self->vk->AcquireProc("vkGetSwapchainImagesKHR", device_handle));
  self->queue_present_khr = reinterpret_cast<PFN_vkQueuePresentKHR>(
      self->vk->AcquireProc("vkQueuePresentKHR", device_handle));

  return self->acquire_next_image_khr != nullptr &&
         self->create_swapchain_khr != nullptr &&
         self->destroy_swapchain_khr != nullptr &&
         self->get_swapchain_images_khr != nullptr &&
         self->queue_present_khr != nullptr;
}
...
static VkPhysicalDevice select_physical_device(FlVulkanManager* self) {
  vulkan::VulkanProcTable& vk = *self->vk;
  ...
  for (const auto& device : devices) {
    ...
    if (surface != VK_NULL_HANDLE) {
      VkResult surface_result = self->get_physical_device_surface_support_khr(
...
static bool initialize_vulkan(FlVulkanManager* self) {
  ...
  auto enumerate_instance_version =
      reinterpret_cast<PFN_vkEnumerateInstanceVersion>(
          self->vk->GetInstanceProcAddr(VK_NULL_HANDLE,
                                        "vkEnumerateInstanceVersion"));
  if (enumerate_instance_version != nullptr) {
    if (enumerate_instance_version(&self->vulkan_version) != VK_SUCCESS) {
      self->vulkan_version = VK_API_VERSION_1_0;
    }
  }
  ...
  if (!load_instance_wsi_procs(self)) {
    FML_LOG(ERROR) << "Failed to load Vulkan WSI instance functions.";
    return false;
  }
  ...
  if (self->window != nullptr) {
    ...
    if (GDK_IS_X11_DISPLAY(gdk_window_get_display(self->window))) {
      ...
      if (self->create_xlib_surface_khr == nullptr) {
        FML_LOG(ERROR) << "vkCreateXlibSurfaceKHR not available.";
        return false;
      }
      result = self->create_xlib_surface_khr(self->instance, &surface_info,
    ...
    if (GDK_IS_WAYLAND_DISPLAY(gdk_window_get_display(self->window))) {
      ...
      if (self->create_wayland_surface_khr == nullptr) {
        FML_LOG(ERROR) << "vkCreateWaylandSurfaceKHR not available.";
        return false;
      }
      result = self->create_wayland_surface_khr(self->instance, &surface_info,
                                                nullptr, &self->surface);
      if (result != VK_SUCCESS) {
  ...
  self->physical_device = select_physical_device(self);
  ...
  if (!load_device_wsi_procs(self)) {
    FML_LOG(ERROR) << "Failed to load Vulkan WSI device functions.";
    return false;
  }
...
static void fl_vulkan_manager_dispose(GObject* object) {
  ...
  if (self->swapchain != VK_NULL_HANDLE &&
      self->destroy_swapchain_khr != nullptr) {
    self->destroy_swapchain_khr(self->device, self->swapchain, nullptr);
    self->swapchain = VK_NULL_HANDLE;
  }
  self->swapchain_images.clear();

  if (self->surface != VK_NULL_HANDLE && self->destroy_surface_khr != nullptr) {
    self->destroy_surface_khr(self->instance, self->surface, nullptr);
...
static void fl_vulkan_manager_init(FlVulkanManager* self) {
  new (&self->vk) fml::RefPtr<vulkan::VulkanProcTable>();
  new (&self->enabled_instance_extensions) std::vector<const char*>();
  new (&self->enabled_device_extensions) std::vector<const char*>();
  new (&self->deferred_deletions)
      std::vector<std::pair<VkImage, VkDeviceMemory>>();
  new (&self->swapchain_images) std::vector<VkImage>();
  new (&self->present_resources)
      std::vector<FlVulkanManager::PresentImageResources>();
  ...
  self->surface = VK_NULL_HANDLE;
  self->get_physical_device_surface_support_khr = nullptr;
  self->get_physical_device_surface_capabilities_khr = nullptr;
  self->get_physical_device_surface_formats_khr = nullptr;
  self->get_physical_device_surface_present_modes_khr = nullptr;
  self->create_xlib_surface_khr = nullptr;
  self->create_wayland_surface_khr = nullptr;
  self->destroy_surface_khr = nullptr;
  self->acquire_next_image_khr = nullptr;
  self->create_swapchain_khr = nullptr;
  self->destroy_swapchain_khr = nullptr;
  self->get_swapchain_images_khr = nullptr;
  self->queue_present_khr = nullptr;
...
gboolean fl_vulkan_manager_ensure_swapchain(FlVulkanManager* self,
  ...
  VkResult result = self->get_physical_device_surface_capabilities_khr(
  ...
  result = self->create_swapchain_khr(self->device, &create_info, nullptr,
                                      &self->swapchain);
  if (old_swapchain != VK_NULL_HANDLE) {
    self->destroy_swapchain_khr(self->device, old_swapchain, nullptr);
  }
  if (result != VK_SUCCESS) {
  ...
  result = self->get_swapchain_images_khr(self->device, self->swapchain,
...
FlutterVulkanImage fl_vulkan_manager_acquire_image(FlVulkanManager* self,
  ...
  VkResult result = self->acquire_next_image_khr(
...
gboolean fl_vulkan_manager_present_image(FlVulkanManager* self,
  ...
  // Skia leaves swapchain images in COLOR_ATTACHMENT_OPTIMAL.
  const VkImageLayout old_layout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  ...
  result = self->queue_present_khr(self->queue, &present_info);
  g_mutex_unlock(&self->queue_mutex);
